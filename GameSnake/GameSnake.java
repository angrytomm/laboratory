...
public class GameSnake {
	...
	final String GAME_OVER_MSG = "ALIAGA PETUX";
	...
	class Snake {
		...
		void move() {
			...
			if (direction == UP) { y --; }
			if (direction == DOWN) { y++; }
			if (x > FILED_WIDTH - 1) { x = 0; }
			if (x < 0) { x = FILED_WIDTH - 1;}
			if (y > FILED_HEIGHT - 1) { y = 0; }
			if (y < 0) { y = FILED_HEIGHT -1; }
			gameOver = isInsideSnake(x,y); // проверка внтру ли змеки?
	...
	class Food extends Point {
		...
		void next() {
			int x,y;
			do {
				x = random.nextInt(FILED_WIDTH);
				y = random.nextInt(FILED_HEIGHT);
	...
	public class Canvas extends JPanel {
		...
		public void paint(Graphics g) {
			...
			if(gameOver) {
				...
				g.drawString(GAME_OVER_MSG, (FILED_WIDTH * POINT_RADIUS + FILED_DX - fm.stringWidth(GAME_OVER_MSG))/2, (FILED_HEIGHT * POINT_RADIUS + FILED_DY)/2);
